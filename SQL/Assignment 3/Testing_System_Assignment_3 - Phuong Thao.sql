DROP DATABASE IF EXISTS Testingsystemassignment_3;
CREATE DATABASE Testingsystemassignment_3;
USE Testingsystemassignment_3;
...
SELECT Fullname FROM `Account` ORDER BY Fullname ASC;
...
SELECT * FROM `cte_dep3` WHERE LENGTH(Fullname) = (SELECT MAX(LENGTH(Fullname)) FROM `cte_dep3`) ORDER BY Fullname ASC;
-- 6
